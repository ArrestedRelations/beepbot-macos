...
const TRACKED_DIRS = ['sidecar/src', 'dashboard/src', 'src'];
...
export class UpdateManager {
  ...
  detectChanges(): Array<{ path: string; hash: string; size: number; action: 'add' | 'modify' | 'delete' }> {
    ...
    try {
      ...
      const gitOutput = execSync('git diff --name-status HEAD~1 2>/dev/null || git diff --name-status --cached 2>/dev/null || echo ""', {
