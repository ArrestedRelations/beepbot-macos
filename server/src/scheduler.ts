...
export class Scheduler {
  ...
  private async tick(): Promise<void> {
    ...
    for (const task of dueTasks) {
      ...
      try {
        ...
        this.db.prepare(
          `INSERT INTO task_runs (id, task_id, status, started_at, duration_ms, manual) VALUES (?, ?, 'ok', ?, ?, 0)`
        ).run(runId, task.id, startedAt, durationMs);

        this.onBroadcast?.({ type: 'scheduler_event', data: { event: 'task_completed', taskId: task.id, name: task.name, durationMs } });
      } catch (err) {
        const durationMs = Date.now() - startMs;
        ...
        this.db.prepare(
          `INSERT INTO task_runs (id, task_id, status, started_at, duration_ms, error, manual) VALUES (?, ?, 'error', ?, ?, ?, 0)`
        ).run(runId, task.id, startedAt, durationMs, errMsg);
  ...
  async runNow(id: string): Promise<void> {
    ...
    try {
      ...
      this.db.prepare(
        `INSERT INTO task_runs (id, task_id, status, started_at, duration_ms, manual) VALUES (?, ?, 'ok', ?, ?, 1)`
      ).run(runId, task.id, startedAt, durationMs);
      this.onBroadcast?.({ type: 'scheduler_event', data: { event: 'task_completed', taskId: task.id, name: task.name, manual: true, durationMs } });
    } catch (err) {
      const durationMs = Date.now() - startMs;
      const errMsg = err instanceof Error ? err.message : String(err);
      this.db.prepare(
        `INSERT INTO task_runs (id, task_id, status, started_at, duration_ms, error, manual) VALUES (?, ?, 'error', ?, ?, ?, 1)`
      ).run(runId, task.id, startedAt, durationMs, errMsg);
  ...
  recordSkip(taskId: string): void {
    this.db.prepare(
      `INSERT INTO task_runs (id, task_id, status, started_at, manual) VALUES (?, ?, 'skipped', datetime('now'), 0)`
    ...
    if (taskId) {
      return this.db.prepare(
        `SELECT r.*, t.name as task_name FROM task_runs r
         ...
         WHERE r.task_id = ? ORDER BY r.created_at DESC LIMIT ?`
