 ...
 * self-modified by the agent. When the agent edits server code, only
...
if (!process.env.ANTHROPIC_API_KEY) {
  ...
  if (storedKey) {
    ...
    console.log('[agent-runtime] Restored Anthropic API key from provider_keys');
  }
}
...
let currentChatIsSystem = false;
let chatQueue: Array<{ content: string; voice?: boolean; id: string; system?: boolean }> = [];
...
async function runChat(content: string, requestId: string, voice?: boolean, system?: boolean): Promise<void> {
  if (chatRunning) {
    chatQueue.push({ content, voice, id: requestId, system });
    return;
  }

  chatRunning = true;
  currentChatIsSystem = system ?? false;
  ...
  try {
    ...
    // Don't broadcast cron prompt content to WebSocket clients
    if (!system) {
    ...
    const eventHandler = (event: AgentEvent): void => {
      // For system/cron turns, suppress content events from reaching the UI
      // Still forward status/done/error so UI knows agent is busy
      if (currentChatIsSystem) {
        if (event.type === 'text' || event.type === 'thinking' || event.type === 'tool_call' || event.type === 'tool_result' || event.type === 'sub_agent') {
          return;
        }
      }
      // Forward agent events to the API server via IPC
    ...
    await agent.chat(content, eventHandler, { system });
    ...
    chatRunning = false;
    currentChatIsSystem = false;
  }

  // Process queued messages
  if (chatQueue.length > 0) {
    const next = chatQueue.shift()!;
    void runChat(next.content, next.id, next.voice, next.system);
  }
}
...
ipc.handle('chat', async (payload) => {
  const { content, voice, id, system } = payload as { content: string; voice?: boolean; id: string; system?: boolean };
  if (!content) throw new Error('Missing content');
  void runChat(content, id, voice, system);
  return { queued: true };
});
...
ipc.handle('switch_model', async (payload) => {
  const { model } = payload as { model: string };
  if (activeAgent) {
    await activeAgent.switchModel(model);
  }
...
ipc.handle('get_models', async () => {
  if (activeAgent) {
    const models = await activeAgent.getSupportedModels();
    return { models };
  }
  return { models: ['sonnet', 'opus', 'haiku'] };
});

// List full model details from Anthropic API
ipc.handle('list_api_models', async () => {
  const apiKey = process.env.ANTHROPIC_API_KEY || getProviderKey(db, 'anthropic');
  if (!apiKey) return { apiModels: null };
  try {
    ...
    if (res.ok) {
      ...
      if (data.data) {
        return { apiModels: data.data.map((m: { id: string; display_name: string }) => ({ id: m.id, displayName: m.display_name })) };
      }
    }
    return { apiModels: null };
  } catch {
    return { apiModels: null };
  }
});

// Set sub-agent models
ipc.handle('set_sub_agent_models', async (payload) => {
  const { models } = payload as { models: Record<string, string> };
