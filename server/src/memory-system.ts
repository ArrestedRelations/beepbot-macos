...
function getMemoryDir(): string {
  return join(getDataDir(), 'workspace', 'memory');
}
...
/** Model ID mapping for short names */
...
/** Read a model setting from DB by key, defaulting to haiku */
function getModelId(db: Database.Database, settingKey: string): string {
  const row = db.prepare('SELECT value FROM settings WHERE key = ?').get(settingKey) as { value: string } | undefined;
  const shortName = row?.value || 'haiku';
  return MODEL_IDS[shortName] || shortName;
}

/** Make an API call using the OAuth token as x-api-key */
async function callModel(oauthToken: string, model: string, systemPrompt: string, userMessage: string): Promise<HaikuResponse> {
...
Only output FACT lines. No other text.`;
    ...
    const extractionModelId = getModelId(db, 'memory_extraction_model');
...
export async function runDailySynthesis(db: Database.Database): Promise<void> {
  ...
  try {
    ...
    try {
      const synthModelId = getModelId(db, 'daily_synthesis_model');
